...
func TestPostCustomerInvoicesRequest(t *testing.T) {
	...
	req.RequestBody().Dimensions = []hogia_v2.Dimensions{{DimensionNumber: "1", DimensionLevel: 1}}
	req.RequestBody().SequenceID = "6081157c-410e-42f0-8e72-67e970e4d1a5"
	req.RequestBody().InvoiceHeader.InvoiceDate = hogia_v2.DateTime{time.Date(2025, 02, 13, 0, 0, 0, 0, time.UTC)}
	req.RequestBody().InvoiceHeader.InvoiceExpirationDate = hogia_v2.DateTime{time.Date(2025, 02, 18, 0, 0, 0, 0, time.UTC)}
	...
	req.RequestBody().InvoiceHeader.BillingAddress = hogia_v2.BillingAddress{Street: "Geraltgatan 12", StreetExtraLine: "2:B", ZipCode: "44453", City: "Stenungsund", Country: "Sverige"}
	req.RequestBody().InvoiceHeader.Customer = hogia_v2.CustomerForInvoice{Name: "Kund AB", Number: "1234", IdentificationNumber: "112233-1234", VatNumber: "SE112233123401", CountryCode: "SE"}
	...
	req.RequestBody().CurrencyInfo = hogia_v2.CurrencyInfo{CurrencyProfitsAccountNumber: 3960, CurrencyLossesAccountNumber: 7960, ExchangeRate: 1, RoundingFactor: 0, Currency: hogia_v2.Currency{Decimals: 2, Code: "SEK"}, AccountingCurrency: hogia_v2.AccountingCurrency{Decimals: 2, Code: "SEK"}}
	req.RequestBody().InvoiceLines = []hogia_v2.InvoiceLines{{UnitPrice: 100, UnitType: "st", Quantity: 1, Vat: hogia_v2.Vat{Percent: 0.25, AccountNumber: 1920}, SalesAccountNumber: 3011, ArticleType: "service", RutRotWorkTypeName: "no-category", StockAccountNumber: 1400, CostAccountNumber: 3590, CostAmount: 77, Dimensions: []hogia_v2.Dimensions{{DimensionNumber: "1", DimensionLevel: 0, AccountType: 0}}, ProjectNumber: "10", DiscountPercent: 0, Name: "Service", Type: "InvoiceLine"}}
	// req.RequestBody().RutRot.RutRotApplicants = []hogia_v2.RutRotApplicants{{SequenceID: "c540e767-2887-4241-b0c1-f0d17736bfb5", IdentificationNumber: "690409-1839", Name: "Tord Österberg", Amount: 562, ApplicationNumber: 0, UtcExportedDate: hogia_v2.DateTime{time.Date(2025, 02, 13, 0, 0, 0, 0, time.UTC)}}}
	// req.RequestBody().RutRot.RutRotWorkTypeLines = []hogia_v2.RutRotWorkTypeLines{{MaterialCost: 2000, WorkHours: 1, Name: "Service", CustomerInvoiceID: "00000000-0000-0000-0000-000000000000"}}
	...
	// req.RequestBody().RutRot.Address = hogia_v2.Address{Street: "Geraltgatan 12", StreetExtraLine: "2:B", ZipCode: "44453", City: "Stenungsund", Country: "Sverige"}
	// req.RequestBody().RutRot.OtherCosts = 0
	// req.RequestBody().RutRot.TotalWorkCost = 2375
	req.RequestBody().PaymentChannels = []hogia_v2.PaymentChannels{{Type: "BankGiro", Value: "123-4567", IsPrimary: true}}
	...
	req.RequestBody().Organization.Address = hogia_v2.Address{Street: "Geraltgatan 12", StreetExtraLine: "2:B", ZipCode: "44453", City: "Stenungsund", Country: "Sverige"}
	req.RequestBody().Organization.ContactChannels = []string{"0303-12345", "contact@organization.com"}
	req.RequestBody().Organization.TypedContactChannels = []hogia_v2.TypedContactChannels{{Value: "contact@organization.com", ContactChannelType: 1}}
